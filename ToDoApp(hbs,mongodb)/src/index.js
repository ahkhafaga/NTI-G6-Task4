...
app.post('/addData', (req, res)=>{
    ...
    dbconnection(db=>{
        ...
        if (data.id){
            db.collection('tasks').updateOne({_id:new ObjectID(data.id)},{$set: {title:data.title, description:data.description}}, (err,data)=>{
...
app.get('/edit/:id', (req,res)=>{
    ...
    dbconnection(db=>{
        ...
        db.collection('tasks').findOne({_id:new ObjectID(id)}, (err,data)=>{
            if(err) res.render('404', {error:'no data'})
            else res.render('add', {data:{id:data._id, title:data.title, description:data.description}})
        })
    })
